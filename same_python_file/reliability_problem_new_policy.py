...
while total_hours_of_operation > total_clock_hour:
    ...
    # find bearing 1 clock from generated random num
    for i in range(len(bearing_random_digits)):
        if bearing_random_num_1 in bearing_random_digits[i]:
            current_clock = bearing_life[i]
            break
    
    # find bearing 2 clock from generated random num
    for i in range(len(bearing_random_digits)):
        if bearing_random_num_2 in bearing_random_digits[i]:
            current_clock = min(bearing_life[i] , current_clock)
            break

    # find bearing 3 clock from generated random num
    for i in range(len(bearing_random_digits)):
...
print("Time spent in bearing replacement =", total_change*three_bearing_change_time_minutes , "minutes")
print("Total downtime =" , total_delay_minutes+(total_change*three_bearing_change_time_minutes) , "minutes")

cost_of_downtime = cost_of_downtime_per_minutes*(total_delay_minutes+(total_change*three_bearing_change_time_minutes))
print("Cost of downtime = Rs.",  cost_of_downtime)
cost_of_repairman = total_change*three_bearing_change_time_minutes*cost_of_repairman_per_hour/60
print("Cost of repairman = Rs.",  cost_of_repairman)
print("\n")
print("Total bearing replaced =", total_change*3)
cost_of_bearing = cost_of_one_bearing*total_change*3
